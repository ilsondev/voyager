...
<div class="col-sm-9 col-md-9 table-container">
    ...
    <table id="table-log" class="table table-striped">
      ...
      @foreach($logs as $key => $log)
        <tr data-display="stack{{{$key}}}">
          <td class="text-{{{$log['level_class']}}} level"><span class="{{ $log['level_img'] == 'warning' ? 'voyager-warning' : 'voyager-info-circled' }}"
                                                           aria-hidden="true"></span> &nbsp;{{$log['level']}}</td>
          <td class="text">{{$log['context']}}</td>
          <td class="date">{{{$log['date']}}}</td>
          <td class="text">
            @if ($log['stack']) <a class="float-end expand btn btn-default btn-xs"
                                   data-display="stack{{{$key}}}"><span
                  class="voyager-search"></span></a>@endif
            {{{$log['text']}}}
  ...
  <div>
    @if($current_file)
      <a href="?download={{ base64_encode($current_file) }}"><span class="voyager-download"></span>
        {{ __('voyager::compass.logs.download_file') }}</a>
      -
      <a id="delete-log" href="?del={{ base64_encode($current_file) }}"><span
            class="voyager-trash"></span> {{ __('voyager::compass.logs.delete_file') }}</a>
      @if(count($files) > 1)
        -
        <a id="delete-all-log" href="?delall=true"><span class="voyager-trash"></span> {{ __('voyager::compass.logs.delete_all_files') }}</a>
      @endif
    @endif
  </div>
</div>
