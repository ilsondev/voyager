...
**Notas de execução:** `laravel/browser-kit-testing` e `orchestra/testbench-browser-kit` foram **mantidos intencionalmente** no `composer.json` nesta fase (não são escopo da Fase 1). Verificado via Packagist: `orchestra/testbench-browser-kit` já tem release v11.0 (compatível com `orchestra/testbench ^11.0` e `laravel/browser-kit-testing ^7.2.7`), e as constraints no `composer.json` são abertas (`>=`), então `composer update` deve conseguir resolver para essas versões sem conflito — **não é um bloqueador para a Fase 2**. A remoção desses pacotes continua sendo objetivo da Fase 3 (modernização de estilo de teste), mas não é mais urgente para destravar as fases seguintes. O CI (`.github/workflows/*.yml`) foi deixado intocado de propósito — é escopo da Fase 6, e atualizá-lo agora deixaria o pipeline vermelho de forma enganosa antes da Fase 2 estar pronta.
